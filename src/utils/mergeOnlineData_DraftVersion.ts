...
export function mergeOnlineData(
    ...
    memos.value.splice(startIndex, endIndex - startIndex, ...onlineData)
    return [startIndex, startIndex + onlineData.length]
}

// this function may have problem when there are memos with the same displayTime in different batch responses
// but it's unlikely to happen in normal use case, therefore won't handle this case
// eg.
// there are some memos with the same displayTime:
// init: [memo1, memo2, memo3]
// batch1: [memo4, memo5]
// batch2: [memo6, memo7]
// after merge from batch1, the result will be [memo4, memo5], [memo1, memo2, memo3] will be deleted because they aren't in the online data.
// then merge from batch2, the result will be [memo6, memo7], [memo4, memo5] will be deleted because they aren't in the batch2.
// to solve this problem, we can use a map to store all the online data we get before, and check if the memo is in the map before deleting it.
